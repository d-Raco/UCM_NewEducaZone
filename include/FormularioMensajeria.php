...
class FormularioMensajeria extends Form
{
    private $tutor;
    private $profesor;
    private $idOrig;
    ...
    public function __construct($tutor, $profesor) {
        parent::__construct('formMensajeria');
    ...
    protected function procesaFormulario($datos)
    {
        ...
        if ( empty($msg) && empty($file['name']) ) {
          ...
          $dao_mensaje = new DAO_Mensajes;
          $mensaje->setId($dao_mensaje->getNumMensajes()+1);
          ...
          if(!empty($file['name'])){
            ...
            $mensaje->setTamanoArchivo($file['size']);
            $mensaje->setTipoArchivo($file['type']);
            move_uploaded_file($file['tmp_name'], $mensaje->getArchivo());
          }
          else{
            $mensaje->setNombreArchivo(null);
            $mensaje->setArchivo(null);
            $mensaje->setTamanoArchivo(0);
            $mensaje->setTipoArchivo(null);
          }

          $dao_mensaje->insertMensaje($mensaje);
        }
        return $result;
    }
}
